...
export class SidebarComponent implements OnInit {
   ...
   intEnroll: number = localStorage.getItem('intEnroll') === null || localStorage.getItem('intEnroll') === "null" ? 0 :Number( localStorage.getItem('intEnroll'));
  menu: any;
  toggle: any = true;
  menuItems: any = [];
  ...
  constructor(private router: Router, public translate: TranslateService, public menuPermissionService: MenuPermissionService) {
    translate.setDefaultLang('en');
  }
//
  ngOnInit(): void {
    // alert(1)
    this.menuPermissionService.MenuPermission(localStorage.getItem('intEnroll'), GlobalComponent.Channel, GlobalComponent.Deviceinfo).subscribe((data: any) => {
  ...
  initActiveMenu() {
    ...
    if (environment.production) {
      ...
      pathName = pathName.replace('/Angular_/angular/interactive', '');
    }
    ...
    if (ul) {
      ...
      let matchingMenuItem = items.find((x: any) => {
        if (environment.production) {
          let path = x.pathname
          path = path.replace('/Angular_/angular/interactive', '');
